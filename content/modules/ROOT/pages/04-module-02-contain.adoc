...
. Go the *Gitea* tab, navigate to `lab/policies/patch-policy.yml` and read through the Rego policy. The company policy will check for:
+
. Maintenance Window
. Backup Verification
. Disk Space
...
. First, examine the execution policy. Open the *Gitea* tab and navigate to `lab/setup/files/opa-policies/dcc-execution-policy.rego`. You will see that the policy specifies the authorized user is `security_engineer`.
...
. Next navigate to **Access Management → Organizations**, and click on the pencil to edit **Default**. Configure the **Policy Enforcement** path to be: `aap/gateway/decision`. Click **Next** and **Finish**.
...
. Test as `admin`: navigate to **Automation Execution → Templates** and try to launch the **Policy-Gated Patching** workflow. The job will fail, if you select the failed template you will see the authorization error.
+
