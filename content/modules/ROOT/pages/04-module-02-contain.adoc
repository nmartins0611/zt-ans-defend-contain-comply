...
. First, we need to look at what the policy actually is. Go the *Gitea* tab, navigate to `lab/policies/patch-policy.yml` and read through the Rego policy. The company policy will check for:
...
. In the *Gitea* tab, navigate to `lab/playbooks/contain/pre-patch-check.yml` and read through the playbook. Uncomment the **Query OPA patch policy** task. 

+
[TIP]
====
. We use `ansible.builtin.stat` to get the backup marker's mtime, then calculate age in hours
. We use `ansible.builtin.systemd` to check httpd's ActiveState
. We get free disk space from `ansible_mounts` (filter for `mount == "/"`, divide `size_available` by 1073741824)
. We POST the four values to `http://central:8181/v1/data/dcc/patch_policy/result` using `ansible.builtin.uri`
. We use `ansible.builtin.fail` with `when: not _opa_response.json.result.allow_patch` to enforce the policy
====
...
+
This policy checks `input.user` against an `authorized_users` set that only contains `security_engineer`. Any other user — including `admin` — is denied with a clear reason.
...
+
* OPA Server hostname: `central`
* OPA server URL: `http://central:8181/v1/data/dcc/execution_policy/result`
* Click **save**

. Next navigate to **Access Management → Organizations**, and click on the pencil to edit **Default**. Configure the **Policy Enforcement** path to be: `aap/gateway/decision`.
...
[source,role="execute"]
----
ssh rhel02 sudo touch /var/log/last-backup-timestamp
----
+
