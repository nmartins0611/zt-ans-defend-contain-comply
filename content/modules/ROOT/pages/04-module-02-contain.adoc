...
However, Benekratis Capital's change management policy requires that patches pass several gates before application: backup verification, maintenance window check, approval, and post-patch compliance validation.
...
* Enforce role-based access control through policy-as-code (OPA user authorization gate)
...
Before writing the playbook, understand the policies that govern patching at Benekratis Capital.
...
* Query OPA and display all gate results (PASS/FAIL) for each of the five gates
* The four system gates (maintenance window, backup, disk space, service health) should all **PASS**
* The `user_authorized` gate will **FAIL** because you are running as `admin` — this is expected and will be addressed in Exercise 4
* The playbook should fail with a clear message showing which gates were violated
* Gates passed: 4/5 (the user authorization gate is the only failure)
...
== Exercise 4: Role-Based Policy Enforcement

[NOTE.why]
====
*Why we do this:* Policy-as-code isn't just about system state — it's about *who* can execute critical operations. In regulated environments, a general administrator shouldn't be able to apply security patches; that's the security team's responsibility. By encoding authorized users into OPA, you enforce separation of duties at the automation layer — not just at the AAP RBAC level, but as a policy gate that applies regardless of AAP permissions. If someone gains admin access, OPA still blocks them from patching unless they're an authorized security engineer.
====

The OPA patch policy includes a `user_authorized` gate that checks who launched the job. Only users in the authorized list (currently `security_engineer`) are permitted to run patching operations. Let's see this in action.

. First, examine the policy gate. Open the *Gitea* tab and navigate to `lab/setup/files/opa-policies/dcc-patch-policy.rego`.
+
Look for the `authorized_patch_users` set and the `user_authorized` rule — only usernames in that set will pass the gate.

. You are currently logged into AAP Controller as `admin`. Navigate to **Automation Execution → Templates** and launch the **CONTAIN - Pre-Patch Check** job template.

. Watch the job output. The playbook sends `requesting_user: admin` to OPA. You'll see:
+
----
FAIL | user_authorized: User 'admin' is authorized for patching operations
----
+
The job **fails** with a policy violation: `admin` is not in the authorized users list.
+
[IMPORTANT]
====
This is the key insight: even though `admin` has full AAP permissions, **OPA denies the operation** at the policy layer. AAP RBAC controls *what you can click*; OPA controls *what the automation is allowed to do*. Defense in depth.
====

. Now log out of AAP Controller (click the user icon in the top right → **Log out**).
...
. Navigate to **Automation Execution → Templates** and launch the **CONTAIN - Pre-Patch Check** job template again.

. This time, all gates pass — including `user_authorized`:
+
----
PASS | user_authorized: User 'security_engineer' is authorized for patching operations
----
+
The policy check completes successfully.

=== Verify

* Running as `admin` → OPA denies with `user_authorized` gate failure
* Running as `security_engineer` → all gates pass, including user authorization
* The same playbook, same system state — the only difference is *who* is running it
...
Stay logged in as `security_engineer` — the workflow's pre-patch check will verify your identity against OPA.
...
* Verify `security_engineer` is authorized via OPA
* Display PASS/FAIL for each gate
+
If all gates pass (including user authorization), the workflow proceeds to the approval node.
...
* The pre-patch check shows `security_engineer` passed the user authorization gate
...
. From the terminal, check the report was generated on rhel01:
+
[source,role="execute"]
----
...
* Demonstrated role-based policy enforcement — `admin` blocked, `security_engineer` authorized
...
* Policy-as-code enforces separation of duties: AAP RBAC controls what you can click, OPA controls what the automation is allowed to do
