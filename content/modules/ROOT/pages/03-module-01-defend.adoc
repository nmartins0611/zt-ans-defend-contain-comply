...
. Click the **Portal - rhel01** tab in the workshop sidebar. This is the Benekratis Capital customer portal — a production application running on `rhel01`. Confirm it is up and publicly accessible.
...
* The Benekratis Capital Portal on rhel01 is accessible in your browser
...
. Click the **Portal - rhel01** tab and confirm the Benekratis Capital portal loads normally.
...
. In the *Gitea* tab, navigate to `lab/extensions/eda/splunk-cve-alert-exercise.yml` and read through the exercise skeleton.

. Open `lab/extensions/eda/splunk-cve-alert.yml` in Gitea, click the pencil icon to edit, and complete the rule using the hints from the exercise skeleton.
+
[TIP]
====
...
. If you get stuck, the complete solution is in Gitea at `lab/extensions/eda/splunk-cve-alert-exercise.yml` — compare the TODO comments with the completed tasks in the solution.

. Commit your changes in Gitea, then sync the EDA project in EDA Controller (*Projects → Defend, Contain, Comply → Sync*).
...
. Wait for the job to complete. It deploys the vulnerable httpd, the Benekratis Capital portal, and configures the errata repository.
...
* The Benekratis Capital Portal on rhel02 is accessible
...
* The Benekratis Capital Portal on rhel02 is **no longer publicly accessible**
