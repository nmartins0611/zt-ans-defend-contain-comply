...
The vulnerability is patched and the service is contained. But remediation alone doesn't satisfy Benekratis Capital's compliance requirements — the security team needs **proof** that the system meets organizational baselines, and the application must be delivered as an immutable, scannable container.
...
Before running the audit, examine the policies that define Benekratis Capital's security baselines.
...
. Examine the full compliance policy (the policy ID uses the file path, not the Rego package name):
+
[source,role="execute"]
----
curl -s http://central:8181/v1/policies/policies%2Fdcc-compliance.rego | python3 -m json.tool
----
+
...
. Review the audit results on rhel01:
+
[source,role="execute"]
----
ssh rhel01 'cat /tmp/compliance-reports/audit-results-pre.json | python3 -m json.tool'
----
+
...
[source,role="execute"]
----
ssh rhel01 "python3 -c '
import json
data = json.load(open(\"/tmp/compliance-reports/audit-results-pre.json\"))
...
for phase in [\"pre\", \"post\"]:
    data = json.load(open(f\"/tmp/compliance-reports/audit-results-{phase}.json\"))
    controls = data.get(\"controls\", [])
    passed = sum(1 for c in controls if c.get(\"pass\"))
    print(f\"{phase.upper()}-HARDENING: {passed}/{len(controls)} controls passed\")
'"
----
+
...
. The report is automatically published to the compliance portal on `central`. Open a browser tab to:
+
[source]
----
http://central:8086/
----
+
Click the report link for this host and date to view the full HTML report with before/after comparison.

. You can also verify the report was generated locally:
+
[source,role="execute"]
----
...
[source,role="execute"]
----
ssh rhel01 'cat /tmp/compliance-reports/compliance-report-$(date +%Y-%m-%d).html | grep -E "Advisory|SELinux|Controls|PASS|FAIL" | head -20'
----
...
[source,role="execute"]
----
ssh rhel01 sudo podman images | grep dcc
----
+
...
[source,role="execute"]
----
ssh rhel01 'sudo podman images --format "table {{.Repository}} {{.Tag}} {{.Size}}" | grep dcc'
----
